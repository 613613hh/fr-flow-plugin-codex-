---
name: fr-release
description: Create and verify a FineReport internal release ZIP containing the complete data-layer CPT directory, selected page CPT files, the latest stored-procedure snapshot, and the project change log.
---
...
Require the project path, release version, and the exact final CPT files. Every release must include the complete `data/` CPT directory, the latest complete stored-procedure snapshot, whether or not the current code change modified it, and the project change log. The change log must include the full version history and the latest release change.
...
```text
reportlets/{project}/data/**/*.cpt
reportlets/{project}/pages/*.cpt
sql/存储过程.sql
CHANGELOG.md
```

The data directory is mandatory and every CPT under it must be included automatically; do not rely on a manually selected single `data_cpt` field. The SQL file is the latest complete stored-procedure snapshot, not an unrelated database bootstrap script. The change log is the project `docs/CHANGELOG.md`, copied as `CHANGELOG.md` at the archive root. Never include `.jsx`, `.mjs`, backups, logs, mock data, node modules, or other documentation in the final archive.
...
- Confirm the project `data/` directory exists and every `*.cpt` file under it is included in the archive.
...
- Inspect the ZIP listing and fail if any data CPT is missing or any extra file is present.
